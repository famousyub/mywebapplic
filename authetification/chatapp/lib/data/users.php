...
$mysqli = new mysqli("localhost", "root", "", "myarticles");

if ($mysqli->connect_error) {
    die("Connection failed: " . $mysqli->connect_error);
}

$query = "SELECT fullname, id, password FROM userauth";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $user['userName'] = $row['fullname'];
        $user['password'] = $row['password'];
        $users['userRole'] = AJAX_CHAT_USER;
        $users['channels'] = array(0,1);
